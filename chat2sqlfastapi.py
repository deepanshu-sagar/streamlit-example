...
@app.post("/process")
async def process(featureName: str, inputQuery: str):
    csql = ChatSql()
    csql.llm = OpenAI(openai_api_key=input.api_key)  # update the API Key
    # Read data.txt and info.json based on featureName
    data = read_data_file(featureName)
    info = read_info_file(featureName)
    # Process the inputQuery and generate the SQL query
    query = csql.prompt_to_query(inputQuery)
